...
class ProjectSession:
    ...
    def _tracking_config_from_dict(payload: dict[str, object] | None) -> TrackingConfig:
        ...
        return TrackingConfig(
            ...
            search_margin=float(payload.get("search_margin", 2.4)),
            expanded_search_margin=float(payload.get("expanded_search_margin", 5.5)),
            scale_factors=tuple(float(value) for value in payload.get("scale_factors", [0.9, 1.0, 1.1])),
            detection_threshold=float(payload.get("detection_threshold", 0.50)),
            low_confidence_threshold=float(payload.get("low_confidence_threshold", 0.36)),
            reacquire_threshold=float(payload.get("reacquire_threshold", 0.56)),
            suspect_after_frames=int(payload.get("suspect_after_frames", 3)),
            recovery_after_frames=int(payload.get("recovery_after_frames", 5)),
            max_prediction_frames=int(payload.get("max_prediction_frames", 8)),
            template_update_rate=float(payload.get("template_update_rate", 0.10)),
            stable_update_threshold=float(payload.get("stable_update_threshold", 0.66)),
            marker_confidence_bias=float(payload.get("marker_confidence_bias", 0.58)),
            auto_marker_min_ratio=float(payload.get("auto_marker_min_ratio", 0.12)),
